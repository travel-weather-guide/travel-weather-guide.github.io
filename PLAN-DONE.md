...
## 2025년도 날씨 데이터 추가 (2026-04-05 완료)

### 스크립트 작성
- [x] `scripts/add-2025-data.ts` 작성 — 2025-01-01~2025-12-31만 fetch, 기존 `daily/[regionId]/all.json`에 `years.2025` 병합

### 프론트엔드 수정
- [x] `src/components/weather/YearComparison.tsx` — `YEARS = [2025, 2024, 2023, 2022]`로 확장
- [x] `src/components/region/RegionDetailContent.tsx` — `YEARS` 동일 확장, `selectedYear` 기본값 `2025`로 변경

### 데이터 생성
- [x] `npx tsx scripts/add-2025-data.ts` 실행 — 67/67 전부 성공
- [x] 샘플 파일(tokyo)에서 `years.2025` 키 존재 확인 — 1월 31일치, 값 정상

### 검증
- [ ] 캘린더 탭 → 2025 탭 선택 동작 확인 (사용자 확인 예정)
- [ ] YearComparison 테이블에 2025 행 표시 확인 (사용자 확인 예정)
...
## 반응형 최적화 — 캘린더/차트 모바일 대응, 터치 UX (2026-04-11 완료)
...
## 25년도 캘린더 데이터 누락 도시 추가 (2026-04-12 완료)

42개 도시 2025년 daily 캘린더 데이터 수집 및 병합

- [x] add-2025-data.ts로 42개 도시 2025년 데이터 수집 + 병합
- [x] 번역 스크립트 확인 (이미 전체 번역 완료 상태)
- [x] 전체 도시 2025 데이터 존재 여부 최종 검증 (42/42 OK)
- [x] npm run build 성공 확인

### 진행 로그
| 시간 | 작업 내용 |
|------|----------|
| 04-12 | add-2025-data.ts --only (42개 도시) 실행, 전체 성공 |
...
## 추천 점수엔진 정합성 수정 + 기계적 reason 재작성 + 전수 데이터 감사 (2026-06-07 완료)

> 선행: SEO 후속 플랜의 A항목(기계적 reason 583 재작성)이 "추천인데 경고 코멘트" 모순 때문에 보류돼 있었음. 근본 원인=점수 엔진이 스모그/극한더위 미반영. 이를 해결하고 전체 정합성까지 감사.

### 점수 엔진 (`scripts/generate-recommendations.py`)
- [x] **대기질 페널티** `AIR_QUALITY_PENALTY` 신설 — 출처검증(IQAir·NASA·정부AQI·피어리뷰) 23개 (지역,월) 패턴. SEVERE 30 / MODERATE 15. 치앙마이·루앙프라방·뉴델리·울란바토르·하얼빈·베이징·시안·카이로·두바이 등. 연도편차 큰 SE아시아 헤이즈는 보수적 제외
- [x] **극한더위 페널티** `calc_heat_penalty` 신설 — tempHigh 기반 ≥40:−28 / 37–40:−25 / 35–37:−15 (월평균 기온점수가 못 잡는 사막·지중해 폭염 보정). cairo·luxor·marrakech·seville·las-vegas·new-delhi·madrid·granada 등 강등
- [x] **전몬순 도시 습열 페널티** `HUMID_HEAT_PENALTY` 신설 — mumbai 4·5월, manila 4월 (건기 해변과 구분한 출처 큐레이션). r5/r4 → r3/r2
- [x] `generate_reason`에 대기질/폭염 마커 추가

### 데이터 정합성 (코멘트 ↔ rating ↔ 기후데이터 전수 감사)
- [x] 1344개 전 항목 결정론적 감사 (추천↔부정코멘트, 비추↔긍정, 폭염과장/과소, 강수·추위 불일치 등 7종)
- [x] **강수 용어 정정 12곳**: penang·krabi 10월 "건기"(실제 445·310mm)→"우기", nha-trang 6월 "우기"(실제 39mm)→"건기", colombo·male·langkawi·yogyakarta 등
- [x] **과장 코멘트 정정**: cappadocia 7·8월 "40°C 폭염"(실제 30°C, 고원지대)→데이터에 맞게
- [x] 모순 94개 → 14개(7개 regex 오탐 + 7개 지중해 성수기, 후자는 웹검증상 코멘트 정확·rating 정당)

### 연쇄 처리
- [x] 583개 기계적 thin reason → 블로거톤 COMMENTS 전면 적용 (원래 A항목 목표 달성)
- [x] **staleness 정합화**: live 데이터가 보정된 날씨 데이터와 어긋나 best 583→재생성 후 정확값. (점수함수 불변, 날씨데이터 보정분이 미반영돼 있던 것)
- [x] 변경된 summary.ko 310개만 collapse→재번역(나머지 dict 보존=기존 번역·705 수동보정 유지), reason 다국어화 → locale 페이지 번역 노출
- [x] `npm run build` exit 0, 6081 페이지, 번역 무결성(잔존 0), 정정 SSR 확인

### 핵심 결정 / 비고
- countries 기후 데이터(rainfall) 직접 수정은 **되돌림** — Open-Meteo API 원본 유지(출처 혼합 금지). 강수 불일치는 코멘트 용어 측에서 해소
- 스키·빙설제·크리스마스마켓 저평점(하얼빈 r1 등)은 버그 아닌 설계 특성(기후 쾌적도 기준) → 목적형 평점은 별도 영역, 미변경
- 번역 스크립트는 dict형 summary를 스킵 → regen 후 재번역하려면 변경 entry를 ko 문자열로 collapse 선행 필요 (워크플로우 갱신점)

### 이월 (사용자·시간 의존)
- [ ] GSC 베이스라인·배포·4~8주 모니터링 → [GATE-2] noindex go/no-go
- [ ] 커스텀 도메인 전환 / 백링크·공유 시딩 / AdSense 시 Open-Meteo 상업플랜

### 진행 로그
| 시간 | 작업 내용 |
|------|----------|
| 2026-06-07 | A항목 보류 원인(점수엔진 스모그/극한더위 미반영) 규명 → 출처검증 페널티 3종 설계·적용 |
| 2026-06-07 | 1344개 전수 정합성 감사 → 강수 용어·과장 코멘트 정정, 모순 94→14 |
| 2026-06-07 | 클린 재생성 + 310건 증분 재번역 + build 검증. countries 데이터 편집은 원복 |
